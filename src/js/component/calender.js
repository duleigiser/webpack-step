...
var Calender = function() {
  ...
  //帮助函数，将个位数补全两位
  function formatToDoubleDigit(n) {
    if (n < 10) {
      n = "0" + n;
    } else {
      n = "" + n;
  ...
  calenderObj = {
    ...
    getDateList: function(currYear, currMonth) {
      ...
      for (var j = -startWeek + 1; j <= nowMonthLong; j++) {
        if (j <= 0) {
          strCont += '<span class="noCont">&nbsp;</span>';
        } else {
          var currMonthTxt = formatToDoubleDigit(currMonth);
          var currDayTxt = formatToDoubleDigit(j);
          strCont += '<span>' + j + '</span>';
        }
      }
    ...
    updateShow: function(currYear, currMonth) {
      ...
      //绑定月份切换按钮
      cache.btnMonthPrev.bind('click', {
        currYear: currYear,
        currMonth: currMonth
      }, showPrevMonth);
      cache.btnMonthNext.bind('click', {
        currYear: currYear,
        currMonth: currMonth
      }, showNextMonth);
      //绑定年份切换按钮
      cache.btnYearPrev.bind('click', {
        currYear: currYear,
        currMonth: currMonth
      }, showPrevYear);
      cache.btnYearNext.bind('click', {
        currYear: currYear,
        currMonth: currMonth
      }, showNextYear);
    }
